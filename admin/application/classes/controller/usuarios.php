...
class Controller_Usuarios extends Controller_Index {
    ...
    public function action_index($mensagem = "", $erro = false) {
        ...
        //BUSCA OS REGISTROS        
        if($this->sessao->get("id_permissao" . $this->nomeSessao) == 1 OR $this->sessao->get("id_permissao" . $this->nomeSessao) == 2){
            $usuarios = ORM::factory('usuarios')->with('permissoes');
        }else{
            $usuarios = ORM::factory('usuarios')->where("USU_ID", "=", $this->sessao->get("id_usuario" . $this->nomeSessao))->with('permissoes');
        }
        ...
        $view->permissao = $this->sessao->get("id_permissao" . $this->nomeSessao);
